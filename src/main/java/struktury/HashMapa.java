...
public class HashMapa {
    public static void main(String[] args) {
        ...
        String[] hasła = napis.split("\n");
        Map<String, ArrayList<String>> dict = new HashMap<>();
        for (String hasło : hasła) {
            String[] wyr = hasło.split(":");
            String klucz = wyr[0];
        ...
        for (Map.Entry<String, ArrayList<String>> entry : dict.entrySet()) {
            System.out.print("Hasło: " + entry.getKey() + " polskie znaczenia: ");
            for (String słowo : entry.getValue())
        ...
        for (Map.Entry<String, ArrayList<String>> entry : dict.entrySet()) {
            for (String słowo : entry.getValue()) {
                if (dictPL.containsKey(słowo)) {
                    lista = dictPL.get(słowo);
                } else {
                    lista = new ArrayList<>();
                }
                lista.add(entry.getKey());
                dictPL.put(słowo, lista);
            }
        }
        System.out.println("=== DIC PL-EN ==");
        for (Map.Entry<String, ArrayList<String>> entry : dictPL.entrySet()) {
            System.out.print("Hasło: " + entry.getKey() + " angielskie znaczenia: ");
            for (String słowo : entry.getValue())
                System.out.print(słowo + ", ");
            System.out.println();
        }
